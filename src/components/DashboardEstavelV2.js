...
function calcularJornada(vendido, metaBase) {
  ...
  return {
    ...
    progresso: Math.max(
      0,
      Math.min(((vendido - anterior) / faixa) * 100, 100)
    ),
  ...
  return (
    <div className={styles.chartArea}>
      <svg
        ...
        viewBox="0 0 700 250"
        role="img"
        aria-label="Evolução acumulada das vendas e metas"
      >
...
function Niveis({ vendido, jornada, compacto: compactoVisual = false }) {
  return (
    <div className={compactoVisual ? styles.levelsCompact : styles.levels}>
      {jornada.niveis.map((nivel) => {
        ...
        const atual = !jornada.completa && nivel.nome === jornada.alvo.nome;
...
function PeriodoDetalhe({ periodo }) {
  return (
    ...
    <section className={styles.dashboard}>
      ...
      <div className={styles.kpiGrid}>
        ...
        <article className={styles.kpi}>
          ...
          <small>{dados.diasLancados} dias com lançamento</small>
        </article>
      </div>
