...
export default function LiderMetas({ telaInicial = "painel" }) {
  ...
  return (
    <main className="app-shell">
      ...
      <nav className="tabs" aria-label="Áreas do Líder Metas" data-tela-inicial={telaInicial}>
        ...
        {podeVerPA && (
          <button
            type="button"
            data-pa-botao
            className={tela === "pa" ? "active" : ""}
            aria-pressed={tela === "pa"}
            onClick={() => setTela("pa")}
          >
            PA das vendedoras
          </button>
      ...
      {tela === "manual" && <ManualUsuario />}
      {tela === "pa" && (
        <PAVendedoras mes={mes} sessao={sessao} perfil={perfil} />
      )}
