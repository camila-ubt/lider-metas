...
function nomeTurno(periodo) { return periodo === "manha" ? "Manhã" : "Noite"; }
function statusHistorico(turno) {
  if (turno.variacaoHistorica === null) return "Sem histórico suficiente do mesmo mês";
  if (turno.variacaoHistorica < -5) return `${percentual.format(Math.abs(turno.variacaoHistorica))}% abaixo da média histórica`;
  if (turno.variacaoHistorica > 5) return `${percentual.format(turno.variacaoHistorica)}% acima da média histórica`;
...
export default function PainelReuniao() {
  ...
  const resumo = useMemo(() => {
    ...
    const turnos = ["manha", "noite"].map((periodo) => {
      ...
      return { periodo, mediaAtual, mediaHistorica, variacaoHistorica, puxa: lojasTurno[0], atencao: lojasTurno[lojasTurno.length - 1] };
    });
  ...
  const sintese = resumo.contexto === "encerrado"
    ? `A operação encerrou o mês com ${percentual.format((resumo.totalVendido / resumo.totalMeta) * 100)}% da Meta, atingindo ${resumo.atingido}. ${faltaProximo > 0 ? `Ficou ${dinheiro.format(faltaProximo)} abaixo da ${resumo.alvo.nome}.` : "O maior nível previsto foi alcançado."}`
  ...
  const motivosNota = [
    ...
    resumo.diferencaLojas <= 8 ? "As lojas estão com desempenho próximo entre si." : `A diferença entre a primeira e a última loja é de ${percentual.format(resumo.diferencaLojas)} pontos percentuais.`,
  ];
    ...
    <section className={styles.wrap}>
      ...
      <div className={styles.grid}>
        <article className={styles.positivo}>
          <h3>Reconhecer</h3>
          <p>{resumo.contexto === "encerrado" ? `${resumo.destaque?.codigo} foi a loja destaque do mês, encerrando com ${percentual.format(resumo.destaque?.percentual || 0)}% da Meta.` : `${resumo.destaque?.codigo} lidera o mês com ${percentual.format(resumo.destaque?.percentual || 0)}% da Meta.`}</p>
          <p>{todasAcima ? "Todas as lojas já superaram a Meta." : "A loja líder está puxando o resultado geral."}</p>
        </article>
        <article className={styles.atencao}>
          <h3>Ponto de atenção</h3>
          <p>{resumo.contexto === "encerrado" ? `${resumo.atencao?.codigo} encerrou com o menor desempenho proporcional: ${percentual.format(resumo.atencao?.percentual || 0)}% da Meta.` : `${resumo.atencao?.codigo} tem o menor desempenho proporcional do ranking.`}</p>
        </article>
      </div>
          ...
          {resumo.turnos.map((turno) => (
            <article key={turno.periodo}>
              <header>
                ...
                <span>{statusHistorico(turno)}</span>
              </header>
              <p>Média diária atual: <b>{dinheiro.format(turno.mediaAtual)}</b></p>
              <p>Média histórica: <b>{dinheiro.format(turno.mediaHistorica)}</b></p>
              <div className={styles.turnoLojas}>
                <p><span>Puxa o resultado</span><b>{turno.puxa?.codigo} · {percentual.format(turno.puxa?.percentual || 0)}%</b></p>
      ...
      <div className={styles.acoes}>
        ...
        {motivosNota.map((item) => <p key={item}>{item}</p>)}
      </div>
    </section>
  );
}
